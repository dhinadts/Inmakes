            ...
            data: (tables) => ListView.builder(
              ...
              itemBuilder: (c, i) {
                ...
                return Card(
                  child: ListTile(
                    title: Text(t.name),
                    subtitle: Text(t.available ? 'Available' : 'Occupied'),
                    trailing: t.available
                        ? null
                        : Text('Order ${t.allocatedToOrderId}'),
                  ),
                );
              },
            ),
                  ...
                  child: ListTile(
                    ...
                    trailing: ElevatedButton(
                      onPressed: () async {
                        ...
                        final tablesSnapshot = await _db
                            .collection('tables')
                        ...
                        if (tablesSnapshot.docs.isEmpty) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('No tables available')),
                          );
                          return;
                        }
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              'Allocated ${tableDoc.data()['name']} to Order ${o.id}',
                            ),
                          ),
                        );
                      },
